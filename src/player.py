...
class Player:
    ...
    def Update(self, keys):
        if keys[0]: self.pos.y -= min(self.pos.y, 360 // GAME_FPS)
        if keys[1]: self.pos.y += min(HEIGHT - self.pos.y - self.pos.height, 360 // GAME_FPS)
        if keys[2]: self.pos.x -= min(self.pos.x, 360 // GAME_FPS)
        if keys[3]: self.pos.x += min(WIDTH - self.pos.x - self.pos.width, 360 // GAME_FPS)
