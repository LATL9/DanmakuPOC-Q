    ...
    def train(self):
        ...
        for f in range(round(FPS * TRAIN_TIME / FRAMES_PER_ACTION)):
            ...
            if TRAIN_MODEL:
                ...
                action = [0 for i in range(FRAMES_PER_ACTION)] # current action; each number = index for action (0 = up, 1 = down, 2 = left, 3 = right)

                while action[FRAMES_PER_ACTION - 1] != 4:
                    q_table[-1].append(self.g.Sim_Update(action))
                    action = [0 for i in range(FRAMES_PER_ACTION)] # current action for state'
                    max_q_value = -9e99
                ...
                action = self.rng.choice([index for (index, item) in enumerate(q_table[-1]) if item == max_q_value]) # index for selected action
